/* =========================================================
   AMOUR — script.js
   Organized into clearly separated modules per the brief:
   1. Navigation
   2. Frame selection
   3. Click-count selection
...
(function () {
  ...
  /* Small helper: Motion is loaded globally from the CDN script in index.html.
     If it fails to load for any reason, we fall back to CSS transitions so
  ...
  ========================================================= */
  const state = {
    frame: null,      // selected frame id
    clicks: null,     // selected number of photos (1-4)
    stream: null,      // active MediaStream
    photos: [],        // captured photo data URLs (in mirrored/display orientation)
    shotIndex: 0,
    sessionToken: 0     // increments on reset to cancel any in-flight countdown loops
  };

  /* =========================================================
     STATIC DATA — frame + click-count definitions
  ========================================================= */
  const FRAMES = [
    {
      id: 'classic',
  ...
  /** Switch the visible screen with a soft fade + rise transition. */
  ...
  /** Stagger-reveal elements marked [data-reveal] / [data-hero-el] inside a screen. */
  ...
  // Wire up every element that declares a navigation target
  document.querySelectorAll('[data-nav-target]').forEach((el) => {
    el.addEventListener('click', () => {
      const target = el.dataset.navTarget;
      if (target === 'screen-camera') {
        if (!state.frame || !state.clicks) return; // guarded by disabled button too
  ...
  /* =========================================================
     MODULE 3 — CLICK-COUNT SELECTION
  ========================================================= */
  ...
  function updateStartButtonState() {
    const ready = Boolean(state.frame) && Boolean(state.clicks);
    startShootingBtn.disabled = !ready;
  }

  /* =========================================================
     MODULE 4 — CAMERA INITIALIZATION
  ========================================================= */
  ...
  /* =========================================================
     MASTER SESSION FLOW — camera -> countdown -> capture -> compose
  ========================================================= */
  ...
  async function startPhotoSession() {
    ...
    // Give the video a beat to stabilize before the first countdown
    await wait(600);
    ...
    for (let i = 0; i < state.clicks; i++) {
      ...
      if (myToken !== state.sessionToken) return;
      capturePhoto();
      await wait(500); // brief pause between shots
    }

    stopCamera();
    cameraHint.textContent = 'Developing your AMOUR original…';
    const finalDataUrl = await composeFinalImage(state.frame, state.photos);
    if (myToken !== state.sessionToken) return;
  ...
  /* =========================================================
     MODULE 6 — PHOTO CAPTURE
  ========================================================= */

  function capturePhoto() {
    const CAPTURE_W = 800;
    const CAPTURE_H = 1000; // 4:5 portrait, matches camera-stage aspect ratio
    ...
    // Cover-fit crop from the source video into the target aspect ratio
    ...
    // Mirror horizontally so the download matches what the user saw on screen
    ctx.save();
  ...
  /** Draw a rounded-rect clipped, cover-fit image. */
  ...
  /** Master composition dispatcher — returns a JPG data URL. */
  ...
  /** Classic / Mini / Love Note share a vertical strip layout with small variations. */
  function drawStripLayout(ctx, images, opts) {
    const { width, photoW, gap, margin, accent, bg, love } = opts;
    const photoH = Math.round(photoW * 1.25); // 4:5 ratio
    const headerH = 46;
    ...
    ctx.font = love ? 'italic 700 30px "Cormorant Garamond", serif' : '700 26px "Playfair Display", serif';
  ...
  /** Polaroid layout — individual tilted prints arranged in a soft grid. */
  function drawPolaroidLayout(ctx, images) {
    ...
    images.forEach((img, i) => {
      ...
      // Polaroid white card with soft shadow
      ctx.save();
  ...
  /** Editorial layout — large clean composition with generous spacing. */
  ...
  /* =========================================================
     MODULE 8 & 9 — JPG GENERATION + DOWNLOAD
  ========================================================= */
  ...
  downloadBtn.addEventListener('click', async () => {
    ...
    // Small delay so the loading state is perceptible and feels intentional
    await wait(500);
  ...
  /* =========================================================
     MODULE 10 — RESET / RESTART
  ========================================================= */

  function resetExperience() {
    state.sessionToken++; // cancel any running countdown loop
    stopCamera();
    state.frame = null;
    state.clicks = null;
    state.photos = [];
